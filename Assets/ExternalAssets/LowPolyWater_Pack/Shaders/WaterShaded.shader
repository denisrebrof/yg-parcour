Shader "LowPolyWater/WaterShaded"
{
    Properties
    {
        ...
        [MaterialToggle] _DirectLight("Direct Light", Float) = 0
        _DirectLightRotation ("Direct Light Rotation", Vector) = (0.0, 0.0, 0.0, 0.0)
    ...
    uniform float4 _DirectLightRotation;
    ...
    half4 calculateBaseColor(v2f input)
    {
        ...
        float4 light_pos;
        if (_DirectLight)
            light_pos = _DirectLightRotation;
        else
            light_pos = _WorldSpaceLightPos0;

        if (0.0 == light_pos.w) // directional light?
        {
            attenuation = 1.0; // no attenuation
            lightDirection = normalize(light_pos.xyz);
        }
        else // point or spot light
        {
            float3 vertexToLightSource = light_pos.xyz - input.posWorld.xyz;
